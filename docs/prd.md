...
- 点击文件名，在中间工作区预览面板中打开该文件；默认只读预览，Markdown 支持轻量编辑
...
- 文件没有改动时，展示全文预览；Markdown 文件允许直接编辑源码
- watcher 触发后只刷新当前文件预览，不全量重算所有文件 diff

**预览渲染：** 非 Markdown 文件使用只读语法化文本排版与 Git 风格行号。大文件采用虚拟滚动，保证几千行文件依然丝滑浏览。Markdown 文件在同一面板内提供轻量源码编辑，支持原始 Markdown 复制与 `cmd+a` 全选。
